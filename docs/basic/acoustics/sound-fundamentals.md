...
位深决定量化精度。16 bit PCM 已能覆盖大多数语音场景，24 bit 更适合录音、后期和需要更高动态范围的链路。算法内部常用 `float32` 表示，方便处理增益和滤波。
...
- 采样率：例如 16000、48000。
- 位深：例如 16 bit、24 bit、32 bit float。
- 声道数：单声道、双声道、麦克风阵列多声道。
- 字节序：小端更常见。
- 帧长：算法通常按 10 ms、20 ms、32 ms 一帧处理。

示例：16 kHz、16 bit、单声道 PCM，每秒数据量为：

```text
16000 samples/s * 16 bit * 1 channel = 256000 bit/s = 32 KB/s
```
