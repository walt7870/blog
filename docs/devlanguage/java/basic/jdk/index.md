...
JDK 是 Java 开发的基础设施。它包含编译器、运行工具、标准类库、诊断工具和 JVM 实现，是 Java 语法、核心 API、构建工具和框架运行的共同前提。

理解 JDK 时要先区分三层：

| 概念 | 作用 | 常见误区 |
| --- | --- | --- |
| JDK | 开发工具包，包含编译、运行、打包、诊断工具和标准库 | 只把它当成运行环境 |
| JRE | 运行 Java 程序需要的运行环境 | 新版本 JDK 分发形态已经弱化单独 JRE |
| JVM | 执行字节码的虚拟机规范和实现 | 把 JVM 和整套 JDK 混为一谈 |

JDK 版本会影响语言语法、标准库、GC、模块系统、工具链和框架兼容性。因此它应属于 Java 基础链路：先知道项目运行在哪个 JDK 上，再学习语法、核心 API、JVM 和框架。

## 阅读入口

| 内容 | 说明 |
| --- | --- |
| 版本发布策略 | 理解传统发布、六个月发布节奏和 LTS 选择 |
| 版本特性 | 快速知道每个版本带来了哪些语言和平台能力 |
| 升级路线 | 具体迁移关注点见 [JDK 版本升级指南](./upgrade.md) |
