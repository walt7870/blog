...
![Spring MVC 请求分派总结图](/spring/summary/mvc-request-dispatch-overview.webp)

总结图用于按失败阶段理解 404、400、415 和返回后的 500。状态码是最终结果，真正的排查入口是请求停在映射、参数解析、业务调用还是响应转换。
...
Spring MVC 是 HTTP 协议和业务方法之间的调度与翻译层。读 MVC 代码时先看请求链，再看 Controller 是否只做协议适配，最后检查校验、异常、拦截器和返回值处理是否集中。排错时依次核对路由、参数解析、校验、拦截链、业务异常和响应转换。
