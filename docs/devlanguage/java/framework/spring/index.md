...
请求到达后，DispatcherServlet 找到 Controller 方法并完成参数转换；Controller 调用注入的 Service；调用经过事务代理后进入业务方法；Repository 访问数据库；返回值再被转换成 JSON。可以把这两条链记成：

```text
启动链：配置 -> 容器 -> Bean -> 代理 -> WebServer
请求链：HTTP -> DispatcherServlet -> Controller -> Service 代理 -> Repository -> JSON
```
