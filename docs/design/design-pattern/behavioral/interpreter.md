...
解释器模式是 GoF 行为型模式之一，适合处理小型语言、规则表达式、查询条件、配置表达式等问题。它在完整编译器领域并不常直接使用，但在规则引擎和表达式解析中很常见。

![解释器模式结构图](/design-pattern/patterns/interpreter.svg)

## 准确理解

解释器模式把语言规则表示成表达式对象，并根据上下文解释执行。它适合语法简单、规则稳定、表达式树规模可控的场景。复杂语言应优先使用成熟解析器、语法分析工具或规则引擎。
...
- 先控制语法范围，避免小语言无限膨胀。
- 表达式节点要小而清晰，组合节点负责连接子表达式。
- 解析阶段和执行阶段分离，便于缓存表达式树。
- 对外部输入要做好校验，避免表达式注入和资源耗尽。
