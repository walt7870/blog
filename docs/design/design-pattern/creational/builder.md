...
建造者模式是 GoF 创建型模式之一，最初用于解决复杂对象构建过程和对象表示分离的问题。随着现代语言的链式 API、不可变对象、配置对象广泛使用，建造者也常用于改善多参数构造函数的可读性。

![建造者模式结构图](/design-pattern/patterns/builder.svg)

## 准确理解

建造者不是为了替代所有构造函数，而是用于构建步骤多、可选参数多、校验规则多的复杂对象。它把“如何一步步组装对象”表达出来，让调用方看清每个参数的含义。

## 问题识别

适合使用建造者的信号：

- 构造函数参数很多，调用时很难看懂每个参数含义。
- 参数有必填、可选、默认值和组合校验。
- 对象构建步骤有顺序要求。
- 希望最终对象保持不可变。
...
- 对象只有两三个字段，构造函数已经清晰。
- 构建过程没有校验和默认值，普通工厂方法即可。
- 业务对象变化很快，过早建造者会增加维护成本。

## 落地要点

- 建造者方法名应表达业务含义，而不是简单复制字段名。
- `build()` 中集中做参数校验和默认值补齐。
- 对核心领域对象，优先让构建结果不可变。
- 不要让建造者长期持有外部资源，它应只负责组装对象。
