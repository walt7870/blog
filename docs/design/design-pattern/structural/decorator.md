...
装饰器模式是 GoF 结构型模式之一，常见于 Java I/O、Web 过滤器、缓存包装、日志包装、加密压缩链路等场景。它体现了组合复用原则：通过包装对象叠加能力，而不是靠继承组合所有能力。

![装饰器模式结构图](/design-pattern/patterns/decorator.svg)

## 准确理解

装饰器在保持接口一致的前提下，为对象动态增加职责。调用方看到的仍是同一个接口，装饰器内部持有被装饰对象，并在调用前后增加逻辑。
