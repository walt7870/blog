...
组合模式是 GoF 结构型模式之一，来源于树形结构建模的常见需求。文件系统、组织架构、菜单、表达式树、组件树都适合用组合模式表达。

![组合模式结构图](/design-pattern/patterns/composite.svg)

## 准确理解

组合模式让客户端以一致方式处理单个对象和组合对象。叶子节点和容器节点实现相同接口，容器节点内部持有子组件列表。它适合“整体和部分具有相同行为语义”的树形结构。
...
- 数据天然是树形结构。
- 调用方希望统一处理叶子和容器。
- 操作需要递归传播，例如渲染、统计、权限检查。
- 新增节点类型不应影响遍历逻辑。

不适合的情况：

- 数据不是树形结构，强行组合会扭曲模型。
- 叶子和容器行为差异极大，统一接口会出现大量空实现。
- 树深度很大且递归操作有性能风险。

## 落地要点

- 组件接口只放叶子和容器都能合理支持的能力。
- 容器节点负责管理子节点，叶子节点不应假装支持添加子节点。
- 递归操作要考虑深度、循环引用和性能。
- 对外暴露树结构时，尽量避免外部随意修改子节点集合。
