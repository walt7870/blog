...
享元模式是 GoF 结构型模式之一，常用于文本编辑器字符对象、地图标记、游戏粒子、连接对象、缓存对象等大量细粒度对象场景。它的核心目标是降低重复对象带来的内存成本。

![享元模式结构图](/design-pattern/patterns/flyweight.svg)

## 准确理解

享元模式通过共享内部状态减少对象数量。内部状态是可共享、稳定、不随场景变化的数据；外部状态是每次使用时传入的数据，例如坐标、颜色、上下文参数。
...
- 先做内存分析，确认对象数量和重复状态确实是问题。
- 共享对象应尽量不可变。
- 外部状态由调用方传入，不应保存在享元对象里。
- 享元工厂负责缓存和复用，避免调用方直接创建。
