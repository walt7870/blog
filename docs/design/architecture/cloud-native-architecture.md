...
云原生不是“把应用部署到云服务器”，而是让应用适合在云和容器平台上弹性运行、自动恢复、快速发布、可观测治理。它关注的是运行方式和工程能力，不只是代码结构。

![云原生架构能力图](/architecture/cloud-native.svg)
...
- 流量波动明显，需要弹性伸缩。
- 需要灰度发布、快速回滚和自动化运维。
- 需要跨环境一致交付，例如开发、测试、生产。

## 不适用场景

- 系统很小，单机或简单容器部署已经足够。
- 团队没有容器、Kubernetes、监控和发布治理能力。
- 为了追求概念引入过重平台，交付反而变慢。
- 应用本身没有健康检查、配置外置、优雅停机等基础能力。
...
2. 补健康检查、优雅停机、配置外置。
3. 接入日志、指标、链路追踪。
4. 建立 CI/CD、灰度发布和回滚流程。
5. 再考虑服务网格、自动扩缩容和平台化能力。

## 检查清单

- 应用是否无状态或状态外置。
- 配置是否通过环境变量、配置中心或 Secret 管理。
- 是否有 readiness 和 liveness 检查。
- 是否能快速回滚。
- 是否能从指标和链路追踪定位故障。
