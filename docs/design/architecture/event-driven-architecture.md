...
事件驱动架构的核心是发布“已经发生的事实”，让其他模块或服务异步响应。它不是把同步调用换成消息队列，而是让系统围绕状态变化解耦。

![事件驱动架构结构图](/architecture/event-driven.svg)

## 适用场景

- 一个业务事实发生后，需要多个后续动作响应。
- 后续动作可以异步完成，不要求强同步返回。
- 服务之间希望降低直接依赖。
- 需要削峰填谷、异步扩展或跨系统集成。
...
| 项 | 建议 |
| --- | --- |
| 命名 | 用过去式事实，例如 `OrderPaid`、`UserRegistered` |
| 内容 | 包含事件 ID、发生时间、聚合 ID、必要业务字段 |
| 幂等 | 消费方用事件 ID 或业务键去重 |
| 失败 | 配置重试、死信队列和告警 |
| 追踪 | 传递 traceId，能从事件查到原始请求 |
