...
DDD 的核心不是战术名词，而是用业务语言划清模型边界。它适合复杂业务系统：业务规则多、概念容易混淆、团队需要和领域专家长期协作。DDD 的目标是让代码结构贴近业务理解，而不是让业务被数据库表结构牵着走。

![DDD 领域驱动设计结构图](/architecture/ddd.svg)
...
- 简单 CRUD 系统，业务规则很少。
- 团队没有时间做领域梳理，只想快速验证原型。
- 只是把实体、值对象、聚合这些名词套到贫血模型上。
