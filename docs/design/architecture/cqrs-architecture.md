...
CQRS 的核心是把“改变状态的写模型”和“读取数据的查询模型”分开。写模型负责业务规则和一致性，读模型负责高效查询和展示。它不是所有系统都需要的架构，只有读写复杂度明显不同或查询压力很大时才值得引入。

![CQRS 架构结构图](/architecture/cqrs.svg)
...
- 团队无法接受读写同步延迟。
- 没有事件、任务或同步机制保障读模型更新。
- 数据一致性要求简单强一致，拆分反而增加成本。

## 模型拆分方式

| 模型 | 职责 | 常见存储 |
| --- | --- | --- |
| 写模型 | 校验命令、执行业务规则、改变状态 | 关系库、领域模型 |
| 事件 | 表达状态变化，驱动读模型更新 | MQ、事件表 |
| 读模型 | 面向页面、报表、搜索的查询视图 | Redis、ES、宽表、物化视图 |

## 实施步骤

1. 先确认查询瓶颈或读写复杂度差异真实存在。
2. 写模型先保持正确，读模型只做展示优化。
3. 用事件或任务同步读模型。
4. 明确同步延迟和一致性提示。
5. 为读模型重建提供脚本或任务。
