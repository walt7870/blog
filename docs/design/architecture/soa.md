...
SOA 的核心是把企业能力服务化，用统一契约让多个系统复用和集成。它更关注企业级集成、服务复用和治理，不等同于微服务。SOA 往往有较强的统一标准和中间层，例如 ESB、API 网关、服务目录。

![SOA 架构结构图](/architecture/soa.svg)

## 适用场景

- 企业内部有多个历史系统需要集成。
- 多个系统复用客户、订单、支付等公共能力。
- 组织需要统一接口标准、协议转换和服务治理。
- 系统之间技术栈差异较大。

## 不适用场景

- 单一产品内部模块拆分，不需要企业级集成。
- 所有请求都经过中心总线，导致瓶颈和强耦合。
- 服务边界只是技术接口，没有业务能力含义。

## 实施要点

- 服务接口要表达业务能力，不只是数据库表 CRUD。
- 统一契约、认证、版本和错误码。
- 总线或网关只做治理，不应承载大量业务逻辑。
- 对高频链路要关注中间层延迟和可用性。
