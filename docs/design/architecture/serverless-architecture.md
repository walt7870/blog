...
Serverless 不是没有服务器，而是开发者不直接管理服务器。系统由事件触发函数运行，平台负责调度、扩缩容、计费和基础运行环境。它适合短任务、事件驱动、流量波动明显的场景。

![Serverless 架构结构图](/architecture/serverless.svg)

## 适用场景

- 图片处理、文件转码、定时任务、Webhook 处理。
- 流量波动大，希望按实际调用计费。
- 业务逻辑短小、无状态、事件触发明确。
- 不想维护长期运行服务。
...
- 函数保持短小、无状态、幂等。
- 状态放到托管数据库、对象存储或队列。
- 对冷启动、超时、重试和并发限制做容量评估。
- 给每个函数补日志、指标和 traceId。
