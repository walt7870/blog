...
微服务不是把系统拆成很多小应用，而是把业务能力、数据所有权、团队责任和部署单元拆开。它解决的是大团队协作、独立交付、故障隔离和独立扩缩容问题，同时会引入分布式系统成本。

![微服务架构结构图](/architecture/microservices.svg)
...
| 判断项 | 推荐做法 |
| --- | --- |
| 业务能力 | 按订单、库存、支付、会员等业务能力拆 |
| 数据归属 | 一个核心数据只能由一个服务负责写 |
| 团队责任 | 一个服务应有明确负责人和发布节奏 |
| 通信方式 | 查询可同步，状态传播优先事件 |
| 一致性 | 服务内强一致，服务间尽量最终一致 |

## 实施步骤

1. 先从模块化单体中识别边界，不要直接按表或代码包拆。
2. 找出数据所有者，禁止多个服务直接写同一张核心表。
3. 先拆低风险、边界清楚、依赖少的服务。
4. 建立服务契约、版本策略、超时、重试和熔断规则。
5. 补齐日志、指标、链路追踪和告警。
